...
namespace PnPeople.Security
{
    public class SEED2
    {
        ...
        {
            return Encrypt(Data, seedKey, false);
        }
		...
		public static byte[] Encrypt(byte[] Data, byte[] seedKey, bool cbcPad)
		{
			...
			return Encrypt(Data, seedKey, cbcPad, seedIV);
		}

        public static byte[] Encrypt(byte[] Data, byte[] seedKey, bool cbcPad, byte[] seedIV)
        {
            ...
            for (int i = 0; i < nOutSize; i += 16)
            {
                ...
                if (cbcPad)
                {
					Buffer.BlockCopy(DataBlock, 0, seedIV, 0, 16);
                }
            }
            return OutData;
        }
        ...
        {
            return Decrypt(Data, seedKey, false);
        }
		...
		public static byte[] Decrypt(byte[] Data, byte[] seedKey, bool cbcPad)
		{
			// CBC ��� �ʱⰪ IV
			byte[] seedIV = new byte[16];
			return Decrypt(Data, seedKey, cbcPad, seedIV);
		}

        public static byte[] Decrypt(byte[] Data, byte[] seedKey, bool cbcPad, byte[] seedIV)
        {
            ...
            for (int i = 0; i < nOutSize; i += 16)
            {
                ...
                if (cbcPad)
                {
					Buffer.BlockCopy(Data, i, seedIV, 0, 16);
                }
            }
