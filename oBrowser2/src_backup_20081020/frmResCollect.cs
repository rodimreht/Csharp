...
namespace oBrowser2
{
	public partial class frmResCollect : Form
	{
		...
		public bool MoveResource()
		{
			...
			{
				// 은하 단위로 자원 운송을 시킬때는 출발지=도착지의 경우 에러 메시지 생략...
				if (galaxyMethod) return true;

				Logger.Log("WARNING: 자원 운송 장애2-2");
				MessageBoxEx.Show("출발지와 도착지의 좌표가 같습니다.",
								  "oBrowser2: 자원 운송 장애2-2",
				                  MessageBoxButtons.OK,
				                  MessageBoxIcon.Exclamation,
				                  10*1000);
				return false;
			...
			{
				Logger.Log("[자원 운송] 최대속도" + (i + 1) + ": " + maxSpeed[i] + ", 거리:" + distance + ", 시간: " + duration[i]);
			}

			int resQuantity = getTotalResources(resInfo.RemainDeuterium);
			double consumption = 0;
			int selectedDuration = 0;
			Logger.Log("[자원 운송] 자원 총량: " + resQuantity);
			...
			for (int i = 0; i < resInfo.Fleets.Length; i++)
			{
				...
				// 선택된 함대의 듀테륨 소모량 계산
				double consumption2 = OB2Util.GetConsumption(allFleets, fleetID, fleetNum, distance, duration[i]);
				consumption += consumption2;

				int useFleet;
				string fleets = OB2Util.GetFleetsText(allFleets, fleetID, fleetNum, out useFleet);
				sb.Append(fleets);
				...
				Logger.Log("[자원 운송] 함대: " + fleetName + "(" + fleetID + ") - " + useFleet + " 대");

				// 남은 자원을 다시 계산
				resQuantity -= OB2Util.GetCapacity(fleetID, useFleet);
				selectedDuration = duration[i];

				// 1순위 함대 수를 다 채웠으면 2순위 함대를 사용하지 않는다
				if (useFleet == fleetNum) break;

				// 선택된 함대의 듀테륨 소모량 다시 계산
				consumption -= consumption2;
				consumption += OB2Util.GetConsumption(allFleets, fleetID, useFleet, distance, duration[i]);
			}
			consumption = Math.Round(consumption) + 1;
			if (resQuantity < 0) resQuantity = 0;
			Logger.Log("[자원 운송] 듀테륨 소모량: " + consumption + ", 남는 자원 총량: " + resQuantity);
