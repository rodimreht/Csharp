...
namespace oBrowser2
{
	...
	public class ExtendedWebBrowser : WebBrowser
	{
		...
		public void ChangeUserAgent(string ua)
		{
		...
		public void SetCookies(Uri targetURL, string cookies)
		{
			...
			{
				if (cookie.IndexOf("prsess_") >= 0)
				{
					InternetSetCookie("http://" + domain + "/", null, cookie);// + "; expires = " + DateTime.Now.AddDays(1).ToUniversalTime().ToString("r"));
					break;
				}
			}
		}
		...
		public class WebBrowserExtendedEvents : UnsafeNativeMethods.DWebBrowserEvents2
		{
			...
			public void BeforeNavigate2(object pDisp, ref object URL, ref object flags, ref object targetFrameName, ref object postData, ref object headers, ref bool cancel)
			{
				...
				//if (!string.IsNullOrEmpty(_Browser.UserAgent))
				//{
				//    if (!((string)headers).Contains(_Browser.UserAgent))
				//    {
				//        headers += string.Format("User-Agent: {0}\r\n", _Browser.UserAgent);
				//        cancel = true;
				//        _Browser.Navigate((string)URL, (string)targetFrameName, (byte[])postData, (string)headers);
				//        return;
				//    }
				//}
			...
			public void NewWindow3(ref object ppDisp, ref bool Cancel, int dwFlags, string bstrUrlContext, string bstrUrl)
			{
