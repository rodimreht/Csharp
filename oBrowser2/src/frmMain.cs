...
namespace oBrowser2
{
	public partial class frmMain : Form
	{
		...
		private int getRefreshRate()
		{
			...
			// 새벽 1시 ~ 7시 사이에는 갱신 주기를 5배로 늘인다. (최소 30분 ~ 최대 180분)
			if (DateTime.Now.Hour >= 1 && DateTime.Now.Hour < 7)
			{
				min *= 5;
				if (min < 30 / _uniX) min = 30 / _uniX;
				max *= 5;
				if (max > 180 / _uniX) max = 180 / _uniX;
			}
			if (max < min) max = min;
		...
		private bool processOverview(string sHtml)
		{
			...
			if (pMain >= 0)
			{
				...
				if (checkEnemyMission(eventHtml, "|attack\"") ||
					checkEnemyMission(eventHtml, "|acs attack\""))	// 공격당하는 중
				{
					addMsg = DateTime.Now.ToString("HH:mm") + " 공격함대 발견!!";

					// SMS, SMTP 송신 및 자동 플릿 세이빙 기능
					checkAttackandSendAlert(eventHtml);
		...
		void _collector_CollectCompleted(object sender, EventArgs e)
		{
			...
			{
				Logger.Log("모든 행성 자원 수집 완료: 플릿 세이빙 예약...");
				_flSavAfterResCollection = false;
				checkFleetSaving(_currentEventHTML);
			}
		}
		...
		void tickTimer_Tick(object sender, EventArgs e)
		{
			...
			// 행성 몇 군데를 더 방문한다: 랜덤 - 스크립트 탐지 방지
			if (_collector != null)
			{
				int num = _collector.ResourceInfos.Length;
				if (num < 2) return;
				Random r = new Random();
				int rnd1 = r.Next(1, num);

				Cursor.Current = Cursors.WaitCursor;
				for (int i = 0; i < rnd1; i++)
				{
					int rnd2 = r.Next(num);
					Uri rndUri = new Uri(_loopURL.OriginalString + "&cp=" + _collector.ResourceInfos[rnd2].ColonyID);
					WebCall.GetHtml(rndUri, ref _loopCookie);
					Logger.Log("추가 페이지 갱신: " + rndUri.OriginalString);
					Application.DoEvents();

					// 최대 3초간 쉰다: 일정한 시각에 발생하는 이벤트 회피(봇 탐지 회피)
		...
		/// 공격 탐지 - SMS/SMTP 송신하기
		/// </summary>
		/// <param name="sHtml">이벤트 HTML.</param>
		private void checkAttackandSendAlert(string sHtml)
		{
			SettingsHelper helper = SettingsHelper.Current;
			...
			while (true)
			{
				...
				if (attPos < 0) attPos = s.IndexOf("|acs attack\"");
				if (attPos < 0) break;

				int attPos1 = s.Substring(0, attPos).LastIndexOf("data-mission-type");
				...
				int posStart = s.Substring(0, attPos1).LastIndexOf("<tr");
				...
				string s2 = s.Substring(posStart, posEnd + 5 - posStart); // <tr ~ </td>
				...
				string hashVal = "<table><tbody>" + s2 + "</tr></tbody></table>";
				...
				if ((attackHash.Length == 0) || (attackHash.IndexOf(hash) < 0))
				{
					...
					{
						arrivalTime = getArrivalTime2(s2);
						if (arrivalTime == DateTime.MinValue)
						{
			...
			while (true)
			{
				...
				if ((attackHash.Length == 0) || (attackHash.IndexOf(hash) < 0))
				{
					...
					if (!string.IsNullOrEmpty(helper.AutoFleetSaving) && helper.AutoFleetSaving == "Y")
					{
						...
						int cPos = s2.IndexOf("detailsfleet\">") + 14;
						if (cPos < 14)
						{
						...
						string enemyNum = s2.Substring(cPos, cPos2 - cPos)
							.Replace("<span>", "")
							.Replace("</span>", "")
							.Replace(".", "").Trim(); // 천 단위 넘어가면 . 구분됨
						int num;
...
//---------------------->???? 행성 형태
		...
		private DateTime getArrivalTime(string sHtml)
		{
			...
			DateTime arrTimeLocal = DateTime.Parse(arrTimeServer).AddHours(-diff);
			
			// 시간 보정: 도착시간이 현재 이전이면 하루를 더해준다.(우리시간으로 08 ~ 10시 사이 도착 시 문제)
			if (arrTimeLocal < DateTime.Now) arrTimeLocal = arrTimeLocal.AddDays(1);
			return arrTimeLocal;
		}

		// 함대 도착시각 얻기2
		private DateTime getArrivalTime2(string sHtml)
		{
			int pos = sHtml.LastIndexOf("data-arrival-time=\"") + 19;
			if (pos < 19) return DateTime.MinValue;
			int pos2 = sHtml.IndexOf("\"", pos + 1);
			if (pos2 < 0) return DateTime.MinValue;

			int diff = SettingsHelper.Current.ApplySummerTime ? -8 : -9;

			// 도착시각(서버시각: timestamp 1413458474)
			string arrivalTime = sHtml.Substring(pos, pos2 - pos);
			DateTime arrTimeServer = OB2Util.FromTimestamp(long.Parse(arrivalTime));
			DateTime arrTimeLocal = arrTimeServer.AddHours(-diff);
			return arrTimeLocal;
		}
		...
		private void doPlanetJob(string action, string coords, int planetType, string bID)
		{
			...
			for (int reCount = 0; reCount < 3; reCount++)
			{
				...
				if (jobForm.DoPlanetJob(action, bID, ref retry))
				{
					...
					if (action.EndsWith("C")) // 취소
						treeUpdate(child, "[" + dd + "] " + actName + " 완료", Color.FromArgb(0x00, 0x00, 0xFF));
					else
					...
					SoundPlayer.PlaySound(Application.StartupPath + @"\newalert.wav");
					MessageBoxEx.Show(actName + (action.EndsWith("C") ? " 작업이 완료되었습니다." : " 작업을 시작했습니다."),
