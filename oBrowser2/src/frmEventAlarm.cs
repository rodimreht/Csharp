	...
	public partial class frmEventAlarm : Form
	{
		...
		private void cmdRegister_Click(object sender, EventArgs e)
		{
			...
			else
			{
				...
				{
					string thePlanet = cboPlanet.Items[cboPlanet.SelectedIndex].ToString();

					if (cboContent.SelectedIndex > 3) // 건설 및 연구
					{
						if (thePlanet.IndexOf("(달)") > 0) // 달에서는 연구 및 건설 제한
						{
							if (cboContent.SelectedIndex > 6) // 연구
							{
								MessageBox.Show("달에서는 연구를 할 수 없습니다.", "이벤트 오류", MessageBoxButtons.OK, MessageBoxIcon.Warning);
								return;
							}
							else if (cboContent.SelectedIndex >= 4 && cboContent.SelectedIndex <= 6) // 건설
							{
								NVItem item = (NVItem)cboDetail.Items[cboDetail.SelectedIndex];
								if (!OB2Util.MOON_BUILDING_IDS.Contains(item.Value))
								{
									MessageBox.Show("달에서 할 수 없는 건설작업 입니다.", "이벤트 오류", MessageBoxButtons.OK, MessageBoxIcon.Warning);
									return;
								}
							}
						}
						...
						{
							if (cboContent.SelectedIndex >= 4 && cboContent.SelectedIndex <= 6) // 건설
							{
								NVItem item = (NVItem)cboDetail.Items[cboDetail.SelectedIndex];
								if (OB2Util.MOON_ONLY_BUILDING_IDS.Contains(item.Value))
								{
									MessageBox.Show("달에서만 가능한 건설작업 입니다.", "이벤트 오류", MessageBoxButtons.OK, MessageBoxIcon.Warning);
									return;
								}
							}
						}
