...
using WebLauncher2.Win32Imports;

#region Blue.Win32Imports

namespace WebLauncher2.Win32Imports
{
	/// <summary>
...
namespace oBrowser2
{
	...
	public class StickyWindow : NativeWindow
	{
		...
		private int borderWidth = 4; // 테두리 두께 보정
		...
		private void Move_Stick(Rectangle toRect, bool bInsideStick)
		{
			...
			if (formRect.Bottom >= (toRect.Top - stickGap) && formRect.Top <= (toRect.Bottom + stickGap))
			{
				if (bInsideStick)
				{
					...
					{	// left 2 right
						formOffsetPoint.X = toRect.Right - formRect.Left - borderWidth;
					}
					if ((Math.Abs(formRect.Left + formRect.Width - toRect.Left) <= Math.Abs(formOffsetPoint.X)))
					{	// right 2 left
						formOffsetPoint.X = toRect.Left - formRect.Width - formRect.Left + borderWidth;
					}
				}

				if (Math.Abs(formRect.Left - toRect.Left) <= Math.Abs(formOffsetPoint.X))
				{	// snap left 2 left
					formOffsetPoint.X = toRect.Left - formRect.Left + borderWidth;
				}
				if (Math.Abs(formRect.Left + formRect.Width - toRect.Left - toRect.Width) <= Math.Abs(formOffsetPoint.X))
				{	// snap right 2 right
					formOffsetPoint.X = toRect.Left + toRect.Width - formRect.Width - formRect.Left - borderWidth;
				}
			}
			if (formRect.Right >= (toRect.Left - stickGap) && formRect.Left <= (toRect.Right + stickGap))
			{
				if (bInsideStick)
				{
					...
					{	// Stick Top to Bottom
						formOffsetPoint.Y = toRect.Bottom - formRect.Top - borderWidth;
					}
					if (Math.Abs(formRect.Top + formRect.Height - toRect.Top) <= Math.Abs(formOffsetPoint.Y) && bInsideStick)
					{	// snap Bottom to Top
						formOffsetPoint.Y = toRect.Top - formRect.Height - formRect.Top + borderWidth;
					}
				}
				...
				{	// top 2 top
					formOffsetPoint.Y = toRect.Top - formRect.Top + borderWidth;
				}
				if (Math.Abs(formRect.Top + formRect.Height - toRect.Top - toRect.Height) <= Math.Abs(formOffsetPoint.Y))
				{	// bottom 2 bottom
					formOffsetPoint.Y = toRect.Top + toRect.Height - formRect.Height - formRect.Top - borderWidth;
				}
			}
		}
		#endregion
