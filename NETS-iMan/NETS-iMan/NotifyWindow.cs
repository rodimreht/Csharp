...
namespace NETS_iMan
{
	...
	public partial class NotifyWindow : Form
	{
		...
		protected Rectangle rGlobText, rGlobTitle;
		protected Rectangle rScreen;
		protected Rectangle rText, rTitle;
		...
		/// Displays the NotifyWindow.
		/// </summary>
		public void Notify(int prevCount)
		{
			...
			rScreen.Height -= prevCount*ActualHeight;
			Height = 0;
			Top = rScreen.Bottom;
			...
			if (Title != null)
			{
				...
				{
					SizeF sz = fx.MeasureString(Title, TitleFont, ActualWidth - rClose.Width - 22, StringFormat);
					rTitle = new Rectangle(11, 12, (int)Math.Ceiling(sz.Width), (int)Math.Ceiling(sz.Height) + 4);
					offset = (int)Math.Max(Math.Ceiling(sz.Height + rTitle.Top + 6), rClose.Bottom + 5);
				}
			}
			else
			{
			...
			rGlobTitle = rTitle;
			if (Title != null) rGlobTitle.Offset(Left, rScreen.Bottom - ActualHeight);
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			if (Title != null)
			{
				...
				using (SolidBrush sb = new SolidBrush(useColor))
					e.Graphics.DrawString(Title, useFont, sb, rTitle, StringFormat);
			}
			...
			if (textPressed)
				...
				useColor = TextColor;
			using (SolidBrush sb = new SolidBrush(useColor))
				e.Graphics.DrawString(Text, useFont, sb, rText, StringFormat);
		}
			...
			if (e.Button == MouseButtons.Left)
			{
				...
				else if (textPressed)
				{
					...
					if (rGlobText.Contains(Cursor.Position))
					{
						...
						Close();
						if (TextClicked != null)
							TextClicked(this, new TextClickedEventArgs(sendText));
					}
				}
				else if (titlePressed)
				{
					...
					if (rGlobTitle.Contains(Cursor.Position))
					{
						...
						if (TitleClicked != null)
							TitleClicked(this, new EventArgs());
					}
				}
			}
			base.OnMouseUp(e);
		...
		[DllImport("UxTheme.dll")]
		protected static extern void DrawThemeBackground(IntPtr hTheme, IntPtr hDC, Int32 partId, Int32 stateId, ref RECT rect,
		                                                 ref RECT clipRect);
		...
		[DllImport("user32.dll")]
		protected static extern bool SetWindowPos(IntPtr hWnd, Int32 hWndInsertAfter, Int32 X, Int32 Y, Int32 cx, Int32 cy,
		                                          uint uFlags);
