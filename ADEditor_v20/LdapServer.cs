...
namespace ADEditor
{
	...
	public class LdapServer
	{
		...
		public static string GetRootEntry(string reptype, string server, string uid, string pwd)
		{
			try
			{
				AuthenticationTypes rootAuthTypes = reptype.ToUpper().Equals("ADAM") ? AuthenticationTypes.Signing | AuthenticationTypes.Sealing | AuthenticationTypes.Secure : AuthenticationTypes.ServerBind | AuthenticationTypes.Secure;

				DirectoryEntry entry = new DirectoryEntry("LDAP://" + server + "/RootDSE", uid, pwd, rootAuthTypes);
				string defaultNamingContext = reptype.ToUpper().Equals("ADAM") ? (string)entry.Properties["NamingContexts"][2] : (string)entry.Properties["defaultNamingContext"][0];
			...
			{
				return "ERROR" + delim + err.ToString();
			}
		}
		...
		public static string GetChildEntry(string reptype, string server, string uid, string pwd, string parent)
		{
			...
			AuthenticationTypes childAuthTypes = reptype.ToUpper().Equals("ADAM") ? AuthenticationTypes.ServerBind | AuthenticationTypes.Secure : AuthenticationTypes.ServerBind | AuthenticationTypes.Secure;
