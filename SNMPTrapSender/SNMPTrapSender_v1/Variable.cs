...
namespace Lextm.SharpSnmpLib
{
    ...
    public class Variable
    {
        ...
        /// <returns></returns>
        internal static IList Transform(Sequence varbindSection)
        {
            IList result = new ArrayList(varbindSection.Items.Count);
            foreach (ISnmpData item in varbindSection.Items)
            {
                ...
                if (null != varbind)
                {
                    if (varbind.Items.Count != 2 || ((ISnmpData)varbind.Items[0]).TypeCode != SnmpType.ObjectIdentifier)
                    {
                        throw new ArgumentException("wrong varbind data");
                    }
                    
                    result.Add(new Variable((ObjectIdentifier)varbind.Items[0], (ISnmpData)varbind.Items[1]));
                }
                else
                {
        ...
        /// <returns></returns>
        internal static Sequence Transform(IList variables)
        {
            IList varbinds = new ArrayList(2 * variables.Count);
