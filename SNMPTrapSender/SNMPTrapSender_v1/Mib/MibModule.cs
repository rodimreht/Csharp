...
namespace Lextm.SharpSnmpLib.Mib
{
    ...
    internal sealed class MibModule
    {
        ...
        private IList _tokens = new ArrayList();
        ...
        private static void ParseEntities(IList tokens, Symbol last, string module, Lexer lexer)
        {
            Symbol temp = last;
            
            IList buffer = new ArrayList();
            IList next = new ArrayList(); // symbol that belongs to next token.
            do
            {
        ...
        private static void ParseEntity(IList tokens, string module, IList buffer, Lexer lexer, ref IList next)
        {
            next.Clear();
            ConstructHelper.Validate((Symbol)buffer[0], buffer.Count == 1, "unexpected symbol");
            ConstructHelper.ValidateIdentifier((Symbol)buffer[0]);
            if (buffer.Count == 2)
            {
                // others
                tokens.Add(ParseOthers(module, buffer, lexer, ref next));
            }
            else if ((Symbol)buffer[1] == Symbol.Object)
            {
                // object identifier
                tokens.Add(ParseObjectIdentifier(module, buffer, lexer));
            }
            else if ((Symbol)buffer[1] == Symbol.ModuleIdentity)
            {
                // module identity
                tokens.Add(new ModuleIdentity(module, buffer, lexer));
            }
            else if ((Symbol)buffer[1] == Symbol.ObjectType)
            {
                tokens.Add(new ObjectType(module, buffer, lexer));
            }
            else if ((Symbol)buffer[1] == Symbol.ObjectGroup)
            {
                tokens.Add(new ObjectGroup(module, buffer, lexer));
            }
            else if ((Symbol)buffer[1] == Symbol.NotificationGroup)
            {
                tokens.Add(new NotificationGroup(module, buffer, lexer));
            }
            else if ((Symbol)buffer[1] == Symbol.ModuleCompliance) 
            {
                tokens.Add(new ModuleCompliance(module, buffer, lexer));
            }
            else if ((Symbol)buffer[1] == Symbol.NotificationType)
            {
                tokens.Add(new NotificationType(module, buffer, lexer));
            }
            else if ((Symbol)buffer[1] == Symbol.ObjectIdentity)
            {
                tokens.Add(new ObjectIdentity(module, buffer, lexer));
            }
            else if ((Symbol)buffer[1] == Symbol.Macro)
            {
                tokens.Add(new Macro(module, buffer, lexer));
            }
            else if ((Symbol)buffer[1] == Symbol.TrapType)
            {
                tokens.Add(new TrapType(module, buffer, lexer));
            }
            else if ((Symbol)buffer[1] == Symbol.AgentCapabilities) 
            {
                tokens.Add(new AgentCapabilities(module, buffer, lexer));
            }
        }
        
        private static IEntity ParseObjectIdentifier(string module, IList header, Lexer lexer)
        {
            ConstructHelper.Validate((Symbol)header[0], header.Count != 4, "invalid OID value assignment");
            ConstructHelper.Expect((Symbol)header[2], Symbol.Identifier);
            return new OidValueAssignment(module, header[0].ToString(), lexer);
        }

        private static IConstruct ParseOthers(string module, IList header, Lexer lexer, ref IList next)
        {
            Symbol current;
        ...
        internal IList Dependents
        {
            get
            {
                return (_imports == null) ? new ArrayList() : _imports.Dependents;
            }
        }
        
        internal static bool AllDependentsAvailable(MibModule module, IDictionary modules)
        {
